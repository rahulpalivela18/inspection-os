...
function requireAdmin(req: Request, res: Response, next: NextFunction) {
  ...
  if (!req.isAuthenticated() || user?.role !== "admin") return res.status(403).json({ message: "Forbidden" });
  next();
}
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    const user = await storage.createUser({ name, email, password: hashed, workspaceId: workspace.id, role: "admin" });
  ...
  app.patch("/api/workspace", requireAdmin, async (req, res) => {
    ...
    if (!parsed.success) return res.status(400).json({ message: parsed.error.errors[0].message });
    const workspace = await storage.updateWorkspace(user.workspaceId, parsed.data);
    res.json(workspace);
  });
  ...
  app.post("/api/checklist-templates", requireAdmin, async (req, res) => {
    const user = req.user as any;
    const parsed = insertChecklistTemplateSchema.safeParse({ ...req.body, workspaceId: user.workspaceId });
    if (!parsed.success) return res.status(400).json({ message: parsed.error.errors[0].message });
  ...
  app.post("/api/projects", requireAuth, async (req, res) => {
    const user = req.user as any;
    const parsed = insertProjectSchema.safeParse({ ...req.body, workspaceId: user.workspaceId });
    if (!parsed.success) return res.status(400).json({ message: parsed.error.errors[0].message });
