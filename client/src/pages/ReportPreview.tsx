...
export default function ReportPreview({ report, project, companyProfile }: ReportPreviewProps) {
  const theme = {
  ...
  const formatArea = (value: number) =>
    new Intl.NumberFormat("en-IN", { maximumFractionDigits: value >= 100 ? 0 : 2 }).format(value);

  const totalAreaSqFt = measuredDimensions.reduce(
    (sum, d) => sum + getAreaInSquareFeet(d.length, d.width, d.unit), 0
  );
      ...
      <div className="min-h-[297mm] print:min-h-0 flex flex-col p-6 md:p-[20mm] bg-white break-after-page relative overflow-hidden print:overflow-visible border-b print:border-0">
        ...
        {hasPageLogo && (
          <div className={pageLogoWrapClass}>
            <img src={project.logoUrl} alt={`${project.title} logo`} className={pageLogoImageClass} />
          </div>
        )}

        <div className="mb-16 pr-24 md:mb-24 md:pr-[160px]">
          <div className={cn("max-w-2xl text-3xl font-extrabold tracking-tight md:text-5xl", theme.text)}>
            {companyProfile.name}
          </div>
        </div>
        ...
        <div className="mt-auto">
          <div className="space-y-4 mb-8 md:mb-12">
            ...
            <p className="text-xl text-slate-500 font-medium">{project.title}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-100 pt-6">
            <div className="space-y-6">
              <div>
                ...
                <p className="text-lg md:text-xl font-bold">{clientDisplayName}</p>
        ...
        <div className="p-6 md:p-[15mm] bg-white flex-1">
            ...
            <div className="mb-12 break-inside-avoid-page">
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <div className="grid grid-cols-[1.3fr_0.8fr_0.9fr_0.9fr_1fr] bg-slate-100 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500">
                  <span>Space</span><span>Unit</span><span>Length</span><span>Width</span><span>Area</span>
                </div>
                ...
                <div className="mb-5 border-b border-slate-100 pb-4 pr-24 md:pr-[160px]">
                  ...
                  <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded-full border border-red-200 bg-red-100 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-red-700">NO</span>
                      {item.severity && (
                        <span className={cn(
          ...
          {failedChecklistItems.length === 0 && (
            <div className="text-center py-20 border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/50">
              <p className="text-slate-300 font-bold italic">No failed findings documented.</p>
            </div>
          )}
        </div>
