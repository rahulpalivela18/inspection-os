...
import {
  ...
  AlertTriangle, CheckCircle2, Circle, X, Image as ImageIcon, ChevronDown, ChevronRight, Ruler, SquareStack, Calculator
} from "lucide-react";
...
export default function ReportEditor() {
  ...
  const saveReport = useCallback((data: any) => {
  ...
  if (isLoading) return (
    <Layout>
      <div className="flex items-center justify-center h-full text-muted-foreground p-8">Loading report...</div>
    </Layout>
  );
  ...
  const measuredDimensionRows = dimensionRows.filter((d) => Number(d.length) > 0 && Number(d.width) > 0);

  const getAreaInSquareFeet = (d: ReportDimension) => {
    const l = Number(d.length), w = Number(d.width);
  ...
  const formatArea = (value: number) => {
    ...
    return new Intl.NumberFormat("en-IN", { maximumFractionDigits: value >= 100 ? 0 : 2 }).format(value);
  };

  const totalAreaSqFt = measuredDimensionRows.reduce((sum, d) => sum + getAreaInSquareFeet(d), 0);
  ...
  const updateDimensionField = (dimensionId: string, field: keyof ReportDimension, value: string) => {
    const next = dimensionRows.map((d) => d.id === dimensionId ? { ...d, [field]: value } : d);
  ...
  const updateChecklistItem = (itemId: string, updates: Partial<ChecklistItem>) => {
    const next = report.checklist?.map((c: ChecklistItem) => c.id === itemId ? { ...c, ...updates } : c);
      ...
      <div className="flex h-screen flex-col bg-background">
        ...
        <div className="border-b border-border bg-white px-4 md:px-6 py-3 flex flex-col lg:flex-row items-start lg:items-center justify-between shrink-0 gap-4 z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 w-full lg:w-auto justify-between lg:justify-end">
            ...
            <Button variant="outline" size="sm" onClick={() => handlePrint()} className="h-9 md:h-10 text-xs md:text-sm px-3 md:px-4">
          ...
          {viewMode === "preview" ? (
            <div className="h-full overflow-y-auto p-4 md:p-8 bg-slate-200/50 flex justify-center print:p-0 print:bg-white print:overflow-visible">
              <div ref={componentRef} className="bg-white shadow-xl w-full max-w-[210mm] min-h-[297mm] p-0 print:shadow-none print:m-0 print:max-w-none origin-top transition-transform sm:scale-100">
                ...
                {project && <ReportPreview report={report} project={project} companyProfile={{ name: workspace?.name || "", logoUrl: workspace?.logoUrl, address: workspace?.address, email: workspace?.email }} />}
              </div>
            </div>
          ) : (
...
function ChecklistView({ report, categories, updateChecklistItem }: {
  report: any;
  categories: string[];
  updateChecklistItem: (id: string, updates: Partial<ChecklistItem>) => void;
}) {
  const [expandedCategories, setExpandedCategories] = useState<Record<string, boolean>>(
    categories.reduce((acc, cat, idx) => ({ ...acc, [cat]: idx === 0 }), {} as Record<string, boolean>)
  );
          ...
          <div key={category} className={catIdx > 0 ? "border-t border-slate-100" : ""}>
            ...
            >
              <div className="flex items-center gap-2">
                {expandedCategories[category] ? <ChevronDown className="h-4 w-4 text-slate-500" /> : <ChevronRight className="h-4 w-4 text-slate-500" />}
                {category}
              </div>
              <div className="flex gap-2 text-xs font-normal">
                <span className="text-green-600 bg-green-50 px-2 py-0.5 rounded-full">
                  {checklist.filter((c) => c.category === category && c.status === "Y").length} Yes
                </span>
                <span className="text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
                  {checklist.filter((c) => c.category === category && c.status === "N").length} No
                </span>
              </div>
            </div>
            ...
            {expandedCategories[category] && (
              <div className="divide-y divide-slate-100">
                {checklist.filter((c) => c.category === category).map((item) => (
                  <ChecklistItemRow key={item.id} item={item} index={checklist.indexOf(item)} update={updateChecklistItem} />
                ))}
              </div>
            )}
          </div>
...
function ChecklistItemRow({ item, index, update }: {
  item: ChecklistItem;
  index: number;
      ...
      <div className="flex flex-col gap-2 pl-8 md:pl-0 shrink-0 w-full sm:w-auto mt-2 md:mt-0">
        <div className="flex flex-wrap items-center gap-2 justify-start md:justify-end">
          <div className="flex bg-slate-100 rounded-lg p-1 border">
            <button
              className={cn("px-3 py-1 text-xs font-bold rounded-md transition-all duration-200", item.status === "Y" ? "bg-green-500 text-white shadow-sm" : "text-slate-500 hover:text-slate-700")}
              onClick={handleYes}
              data-testid={`button-yes-${item.id}`}
            >YES</button>
            <button
              className={cn("px-3 py-1 text-xs font-bold rounded-md transition-all duration-200", item.status === "N" ? "bg-red-500 text-white shadow-sm" : "text-slate-500 hover:text-slate-700")}
              onClick={handleNo}
          ...
          {item.status === (item.failOn ?? "N") && (
            <select
              ...
              onChange={(e) => update(item.id, { severity: (e.target.value || null) as any })}
...
function DimensionsView({ dimensionUnit, dimensionRows, measuredDimensionRows, totalAreaSqFt, totalAreaSqM, formatArea, updateDimensionField, updateDefaultUnit }: any) {
  return (
    <>
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Enter length and width for each room. We calculate total square feet instantly.
            </p>
          </div>
          <div className="grid gap-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:min-w-[220px]">
            ...
            <p className="text-xs text-slate-500">Changing this updates all room inputs.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card className="rounded-3xl border-indigo-100 bg-white/90 p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-indigo-50 p-3 text-indigo-600"><Calculator className="h-5 w-5" /></div>
            <div>
        ...
        <Card className="rounded-3xl border-slate-200 bg-white/90 p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-slate-100 p-3 text-slate-700"><SquareStack className="h-5 w-5" /></div>
            <div>
        ...
        <Card className="rounded-3xl border-slate-200 bg-white/90 p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-emerald-50 p-3 text-emerald-600"><Ruler className="h-5 w-5" /></div>
            <div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {dimensionRows.map((dimension: ReportDimension) => {
          const areaSqFt = (() => {
            const l = Number(dimension.length), w = Number(dimension.width);
            if (!Number.isFinite(l) || !Number.isFinite(w) || l <= 0 || w <= 0) return 0;
